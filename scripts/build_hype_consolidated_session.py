...
# Short, UI-facing marker labels by kind.
_MARKER_TEXT = {"meta_open": "опен", "meta_close": "фул клоуз", "dca_buy": "DCA buy"}


def _norm_symbol(value: Any) -> str:
    """Normalize a symbol to base+quote so different source notations match.

    Sources log the same market in different forms ("HYPE-USDT" vs
    "HYPE/USDT:USDT"); both must dedupe to one logical event. Take the first two
    alphanumeric tokens, e.g. HYPE-USDT -> HYPEUSDT, HYPE/USDT:USDT -> HYPEUSDT.
    """
...
def _marker_from_kind(marker_id: str, ts: str, price: float, kind: str, side: str, symbol: str) -> Dict[str, Any]:
    is_close = kind == "meta_close"
    return {
        "id": marker_id,
        "time": ts,
        "price": float(price),
        "text": _MARKER_TEXT.get(kind, kind),
        "kind": kind,
...
def _order_marker(row: Dict[str, Any], idx: int) -> Optional[Dict[str, Any]]:
    ...
    kind = "meta_close" if order_type in {"CLOSE", "EXIT", "CLOSE_PARTIAL"} else "dca_buy"
    if "lead_open_position_detected" in reason:
        kind = "meta_open"
    return _marker_from_kind(
        str(row.get("order_id") or f"order-{idx}"),
        ts,
        float(price),
        kind,
        str(row.get("side") or ""),
...
def _read_event_csv(path: Path) -> List[Dict[str, Any]]:
    ...
    for idx, row in df.iterrows():
        ...
        if not ts or price is None:
            continue
        event_type = str(row.get("type") or row.get("kind") or "").lower()
        if "open" in event_type:
            kind = "meta_open"
        elif "close" in event_type or "exit" in event_type:
            kind = "meta_close"
        else:
            kind = "dca_buy"
        out.append(
            _marker_from_kind(
                str(row.get("order_id") or row.get("id") or f"{path.parent.name}-event-{idx}"),
                ts,
                float(price),
                kind,
                str(row.get("side") or ""),
...
def _merge_markers(*groups: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # lead-trader trade mirrored across exchanges (htx/gateio/bingx/mexc) and
    # across snapshots of one live run. Each copy has a unique order_id, a
    # slightly different sub-second timestamp and a per-exchange fill price, so
    # keying on id (or exact ts+price) never collapses them and the chart shows
    # 2-4 arrows stacked on one candle. Collapse them by
    # (minute bucket, kind, side, symbol); keeping side+symbol preserves both
    # legs of dual-direction (long+short) strategies. The earliest copy wins.
    ...
    for item in items:
        ts = pd.to_datetime(item["time"], utc=True, errors="coerce")
        minute_bucket = None if pd.isna(ts) else int(ts.timestamp() // 60)
        kind = str(item.get("kind") or item.get("text") or "")
        side = str(item.get("side") or "").upper()
        symbol = _norm_symbol(item.get("symbol"))
        if side or symbol:
            key: Tuple[Any, ...] = (minute_bucket, kind, side, symbol)
        else:
            key = (minute_bucket, kind, f"{item['price']:.8f}")
    ...
    event_rows = [
        {
            ...
            "symbol": row.get("symbol") or "HYPE-USDT",
            "price": row.get("price"),
            "qty": "",
            "order_id": row.get("id"),
            "position_id": "",
