...
def prune_db_to_universe(db_path: str, keep_symbols: List[str]) -> None:
    """
    Remove any symbols from DB that are NOT in keep_symbols.
    This prevents global MIN/MAX(datetime_utc) being affected by stale symbols from previous runs,
    which can shift backtest [time range] and cause "missing bars" stalls.
    """
    keep = [str(s).strip() for s in keep_symbols if str(s).strip()]
    if not keep:
        return
    con = sqlite3.connect(db_path)
    cur = con.cursor()
    # Fast path: delete rows not in keep
    qmarks = ",".join(["?"] * len(keep))
    cur.execute(f"DELETE FROM price_indicators WHERE symbol NOT IN ({qmarks})", tuple(keep))
    con.commit()
    con.close()
...
def main():
    ...
    args = ap.parse_args()
    ap.add_argument("--prune-db-to-universe", action="store_true",
                    help="Delete rows for symbols not present in the current universe file. Recommended when reusing a DB path across runs to avoid shifted global time ranges.")
    ...
    # --- Resolve universe to actual market symbols upfront (so pruning and later loops use the same canonical symbol keys) ---
    resolved_markets: List[str] = []
    for raw_sym in symbols:
        try:
            mkt_sym = resolve_market(ex, normalize_token(raw_sym), fmt_bias=args.ccxt_symbol_format)
        except Exception:
            mkt_sym = None
        if mkt_sym:
            resolved_markets.append(mkt_sym)

    # If reusing an output DB, stale symbols from older runs can shift global MIN/MAX(datetime_utc) and stall backtests.
    # We prune DB to the current universe when requested (or when --fresh, pruning is harmless).
    if getattr(args, "prune_db_to_universe", False) or args.fresh:
        prune_db_to_universe(args.output, resolved_markets)

    # Use resolved markets for the actual fetch loop (skip ones that failed to resolve)
