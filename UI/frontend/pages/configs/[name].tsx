...
const Monaco = dynamic(()=>import('@monaco-editor/react'),{ssr:false}); const f=(u:string)=>fetch(u).then(r=>r.json());
export default function P(){ const r=useRouter(); const name=r.query.name as string; const {data,mutate}=useSWR(name?'/api/configs/'+name:null,f); const [txt,setTxt]=useState<string>('');
 ...
 <button onClick={async()=>{const res=await fetch('/api/configs/'+name,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({yaml_text:txt||data.yaml_text})}); alert(res.ok?'Saved':'Failed'); mutate();}}>Save</button></div>;
}
