...
function normalizeSeriesToZero(points: Point[]): Point[] {
  if (!Array.isArray(points) || points.length === 0) return [];
  const first = Number(points[0]?.value);
  if (!Number.isFinite(first)) return points;
  return points.map(p => ({ ...p, value: Number(p.value) - first }));
}

function alignPnlSeries(backtest: Point[], live: Point[]): { backtest: Point[]; live: Point[] } {
  const bt = normalizeSeriesToZero(backtest || []);
  const lv = normalizeSeriesToZero(live || []);
  if (!bt.length && !lv.length) return { backtest: bt, live: lv };

  const btStart = bt.length ? Date.parse(String(bt[0].ts || '')) : Number.POSITIVE_INFINITY;
  const lvStart = lv.length ? Date.parse(String(lv[0].ts || '')) : Number.POSITIVE_INFINITY;
  const commonStartMs = Math.min(btStart, lvStart);
  const commonStartIso = Number.isFinite(commonStartMs) ? new Date(commonStartMs).toISOString() : (bt[0]?.ts || lv[0]?.ts || new Date().toISOString());

  const ensureCommonStart = (src: Point[]) => {
    if (!src.length) return [] as Point[];
    const firstTs = Date.parse(String(src[0].ts || ''));
    const out: Point[] = [];
    out.push({ ts: commonStartIso, value: 0 });
    if (Number.isFinite(firstTs) && firstTs > commonStartMs) {
      out.push({ ts: src[0].ts, value: 0 });
    }
    out.push(...src);
    return out;
  };

  return { backtest: ensureCommonStart(bt), live: ensureCommonStart(lv) };
}
...
export default function BacktestLiveValidationPage() {
  ...
  const alignedPnl = alignPnlSeries(runData?.pnl_chart?.backtest || [], runData?.pnl_chart?.live || []);
