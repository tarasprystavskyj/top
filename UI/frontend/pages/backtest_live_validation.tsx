...
function normalizeAndSortSeries(points: Point[]): Point[] {
  if (!Array.isArray(points) || points.length === 0) return [];
  const withTs = points
    .map(p => ({ ...p, value: Number(p.value), _tsMs: Date.parse(String(p.ts || '')) }))
    .sort((a, b) => {
      const ax = Number.isFinite(a._tsMs) ? a._tsMs : Number.POSITIVE_INFINITY;
      const bx = Number.isFinite(b._tsMs) ? b._tsMs : Number.POSITIVE_INFINITY;
      return ax - bx;
    });
  const firstFinite = withTs.find(p => Number.isFinite(p.value));
  if (!firstFinite) {
    return withTs.map(({ _tsMs, ...rest }) => rest);
  }
  const base = firstFinite.value;
  return withTs.map(({ _tsMs, ...p }) => ({ ...p, value: Number.isFinite(p.value) ? p.value - base : p.value }));
}

function alignPnlSeries(backtest: Point[], live: Point[]): { backtest: Point[]; live: Point[] } {
  return {
    backtest: normalizeAndSortSeries(backtest || []),
    live: normalizeAndSortSeries(live || []),
  };
}
