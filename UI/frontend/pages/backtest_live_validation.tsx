...
function LineChart({ title, series }: { title: string; series: { name: string; color: string; data: Point[] }[] }) {
  ...
  const height = 280;
  const padding = 38;
  const merged = series.flatMap(s => s.data.map((d, i) => ({ x: i, y: Number(d.value) || 0 })));
  ...
  return (
    <div style={{ border: '1px solid #d1d5db', borderRadius: 8, padding: 12, background: '#fff' }}>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 280 }}>
        ...
        {series.map(s => {
          const pts = s.data.map((d, i) => ({ x: i, y: Number(d.value) || 0 }));
...
export default function BacktestLiveValidationPage() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {runData && (
        <>
          {(runData?.pnl_chart?.live || []).length === 0 && runData?.fallback_canvas_url && (
            <div style={{ border: '1px solid #d1d5db', borderRadius: 8, padding: 12, background: '#fff' }}>
              <h4 style={{ marginTop: 0 }}>Fallback canvas (matcher output)</h4>
              <img src={runData.fallback_canvas_url} style={{ width: '100%', maxWidth: 980 }} />
            </div>
          )}
          <LineChart
