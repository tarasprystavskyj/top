...
export default function Run(){ const [cfgs,setCfgs]=useState<any[]>([]); const [cfg,setCfg]=useState(''); const [bars,setBars]=useState(5000); const [job,setJob]=useState<any>(null); const [res,setRes]=useState<any>(null);
 useEffect(()=>{apiFetch('/api/configs').then(r=>r.json()).then(setCfgs)},[]);
 async function start(){ const j=await apiFetch('/api/backtest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cfg_name:cfg,limit_bars:bars})}).then(r=>r.json()); setJob(j); }
 useEffect(()=>{ if(!job) return; const id=setInterval(async()=>{ const st=await apiFetch('/api/jobs/'+job.job_id+'/status').then(r=>r.json()); if(st.status==='done'||st.status==='error'){ const rs=await apiFetch('/api/jobs/'+job.job_id+'/result').then(r=>r.json()); setRes(rs); clearInterval(id);} },1000); return ()=>clearInterval(id); },[job]);
