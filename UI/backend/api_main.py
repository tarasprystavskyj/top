...
def _list_validation_source_files() -> List[Dict[str, Any]]:
    if not os.path.isdir(TV_BACKTEST_SOURCE_DIR):
        return []
    out: List[Dict[str, Any]] = []
    for entry in sorted(os.scandir(TV_BACKTEST_SOURCE_DIR), key=lambda e: e.name.lower()):
...
def _load_validation_dataset(run_id: str) -> Dict[str, Any]:
    ...
    if not bx_df.empty:
        bx_df = bx_df.copy()
        bx_df["dt"] = pd.to_datetime(bx_df.get("Час виконання"), errors="coerce", dayfirst=True, utc=True)
    ...
    live_state = _compute_position_state(bx_df, "Ф’ючерси / Напрямок", "qty", "price", "Відкрити", "Закрити") if not bx_df.empty else {"qty": 0.0, "entry": 0.0}
    ...
    stats = {
        ...
        "comparison": {
            ...
            "matched_share": float(len(matched_df) / len(bx_df)) if len(bx_df) else 0.0,
...
def backtest_live_validation_files():
    return {"files": _list_validation_source_files()}
...
def backtest_live_validation_inspect(payload: Dict[str, Any] = Body(default={})):  # noqa: B008
    ...
    _ensure_under(abs_path, REPO_ROOT)
...
def backtest_live_validation_run(payload: Dict[str, Any] = Body(default={})):  # noqa: B008
    ...
    _ensure_under(tv_path, REPO_ROOT)
    ...
    symbol_safe = (inspect.get("symbol") or "symbol").replace("-", "_")
    meta = {
        "run_id": run_id,
        "run_dir": run_dir,
        "tv_path": tv_path,
        ...
        "match_ready_csv": os.path.join(run_dir, f"{symbol_safe}_trade_history_for_match.csv"),
...
def backtest_live_validation_poll(run_id: str):
    ...
    return {
        ...
        "pnl_chart": {"live": data.get("pnl_chart", {}).get("live", [])},
        "margin_chart": {
