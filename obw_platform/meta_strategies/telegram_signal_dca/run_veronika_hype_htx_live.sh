...
# Veronika/HYPE HTX live profile using the guarded canary runner.
# The runner reads the env file internally and must not print secret values.

ACK_VALUE="I_ACCEPT_REAL_HTX_ORDERS"
if [[ "${VERONIKA_HYPE_HTX_LIVE_ACK:-}" != "${ACK_VALUE}" ]]; then
  echo "Refusing to start live runner. Set VERONIKA_HYPE_HTX_LIVE_ACK=${ACK_VALUE} explicitly." >&2
  exit 2
fi
...
CFG="${CFG:-${REPO_ROOT}/obw_platform/meta_strategies/telegram_signal_dca/configs/htx_veronika_hype_live_110.json}"
OUT_DIR="${OUT_DIR:-/var/www/vps2.happyuser.info/top/top_1/obw_platform/_reports/_live/veronika_hype_htx_110_$(date -u +%Y%m%dT%H%M%SZ)}"
RUN_ID="${RUN_ID:-VERONIKA_HYPE_HTX_110_$(date -u +%Y%m%dT%H%M%SZ)}"
SERVER_PROFILE="${SERVER_PROFILE:-/var/www/vps2.happyuser.info/top/.codex_CLI_laptop_local_philosofy/profiles/top_1.server.sh}"
DEADLINE_UTC="${VERONIKA_HYPE_HTX_DEADLINE_UTC:-}"
