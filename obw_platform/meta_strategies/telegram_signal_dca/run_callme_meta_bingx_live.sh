...
if [[ "${CALLME_META_BINGX_LIVE_ACK:-}" != "${ACK_VALUE}" ]]; then
  echo "Refusing to start live runner. Set CALLME_META_BINGX_LIVE_ACK=${ACK_VALUE} explicitly." >&2
  exit 2
fi
...
OUT_DIR="${OUT_DIR:-/var/www/vps2.happyuser.info/top/top_1/obw_platform/_reports/_live/callme_meta_bingx_54_$(date -u +%Y%m%dT%H%M%SZ)}"
RUN_ID="${RUN_ID:-CALLME_META_BINGX_54_$(date -u +%Y%m%dT%H%M%SZ)}"
SERVER_PROFILE="${SERVER_PROFILE:-/var/www/vps2.happyuser.info/top/.codex_CLI_laptop_local_philosofy/profiles/top_1.server.sh}"
DEADLINE_UTC="${CALLME_META_BINGX_DEADLINE_UTC:-}"
