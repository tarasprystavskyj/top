...
if [[ "${CALLME_AMD_GATEIO_LIVE_ACK:-}" != "${ACK_VALUE}" ]]; then
  echo "Refusing to start live runner. Set CALLME_AMD_GATEIO_LIVE_ACK=${ACK_VALUE} explicitly." >&2
  exit 2
fi
...
CFG="${CFG:-${REPO_ROOT}/obw_platform/meta_strategies/telegram_signal_dca/configs/gateio_callme_amd_live_254.json}"
OUT_DIR="${OUT_DIR:-/var/www/vps2.happyuser.info/top/top_1/obw_platform/_reports/_live/callme_amd_gateio_254_$(date -u +%Y%m%dT%H%M%SZ)}"
RUN_ID="${RUN_ID:-CALLME_AMD_GATEIO_254_$(date -u +%Y%m%dT%H%M%SZ)}"
SERVER_PROFILE="${SERVER_PROFILE:-/var/www/vps2.happyuser.info/top/.codex_CLI_laptop_local_philosofy/profiles/top_1.server.sh}"
DEADLINE_UTC="${CALLME_AMD_GATEIO_DEADLINE_UTC:-}"
