...
First-basket margin-call cleanup remains satisfied with 0 total margin-call events using SUP budget30. Budget32 is a secondary if a slightly higher -30.41% drawdown is acceptable. The latest budget30 ladder isolation did not improve the risk/return balance, so the next useful SUP search should avoid simple ladder tightening and instead test exit behavior or a constrained tuner objective around budget30.
