...
def move_sl_after_tp(fetcher_or_ex, symbol, side, new_trigger_price, old_sl_order_id=None, qty_hint=None):
    ...
    fallback_free = None
    if pos_qty <= 0 and qty_hint is not None:
        try:
            hint_qty = max(0.0, float(qty_hint))
        except Exception:
            hint_qty = 0.0
        if hint_qty > 0:
            fallback_free = max(0.0, hint_qty - reserved)
            if fallback_free > 0:
                pos_qty = max(pos_qty, fallback_free + reserved)
    effective = max(0.0, pos_qty - reserved)
    if fallback_free is not None and fallback_free > effective:
        effective = fallback_free
...
def _calc_free_reduceonly_qty(
    fetcher,
    sym,
    side_close,
    position_mode,
    local_total_qty: Optional[float] = None,
):
    """
    ...
    fallback_free = None
    if available_amt <= 0 and local_total_qty is not None:
        try:
            fallback_local = max(0.0, float(local_total_qty))
        except Exception:
            fallback_local = 0.0
        if fallback_local > 0:
            fallback_free = max(0.0, fallback_local - reserved)
            if fallback_free > 0:
                available_amt = fallback_free + reserved

    free_qty = max(0.0, available_amt - reserved)
    if fallback_free is not None and fallback_free > free_qty:
        free_qty = fallback_free

    fallback_note = ""
    if fallback_free is not None and fallback_free > 0:
        fallback_note = f" fallback={fallback_free:.6g}"
    if BE_DIAG:
        cprint(
            f"[sl->BE diag] {sym} side_close={side_close} posSide={target_pos_side or 'BOTH'} "
            f"available={available_amt:.6g} reserved={reserved:.6g} free={free_qty:.6g}{fallback_note}",
            fg="cyan",
            dim=True,
        )
...
def _place_additional_stop_at_BE(fetcher, sym, rec, position_mode, be_price, now_utc):
    ...
    available_amt, reserved_qty, free_qty = _calc_free_reduceonly_qty(
        fetcher, sym, sl_side, position_mode, local_total_qty=rec.get('qty')
    )
    local_qty = qty
...
def _place_tp_sl_after_open(
    ...
    try:
        ...
        ladder_full = allocate_tp_ladder(qty, [1.0], lot_step, min_qty)
        tp_qty_full = ladder_full[0] if ladder_full else 0.0
        ...
        if (
            ...
            if qty:
                try:
                    frac_target = float(part_tp_qty) / float(qty)
                except Exception:
                    frac_target = 0.0
            frac_target = max(0.0, min(frac_target, 1.0))
            ladder_part = allocate_tp_ladder(
                qty,
                [frac_target, 1.0],
                lot_step,
                min_qty,
            )
            part_qty_eff = ladder_part[0] if ladder_part else 0.0
            remainder_alloc = ladder_part[1] if len(ladder_part) > 1 else max(0.0, qty - part_qty_eff)
            if part_qty_eff <= 0:
                if min_qty and float(part_tp_qty) < min_qty:
                    _dbg(
                        "ptp_skip",
                        sym,
                        f"reason=min_qty qty={float(part_tp_qty):.6g} min={min_qty}",
                    )
                else:
                        ...
                        if heat >= open_heat_min:
                            ...
                            try:
                                ...
                                if od:
                                    ...
                                    if oid:
                                        cprint(
                                            f"[be-plan] placed plan on exchange id={oid}",
                                            fg='cyan',
                                            dim=True,
                                        )
                                    else:
                                        cprint(
                                            "[be-plan] placed plan on exchange",
                                            fg='cyan',
                                            dim=True,
                                        )
                                else:
                                    cprint(
                                        "[be-plan] failed to place plan on exchange",
                                        fg='yellow',
                                        dim=True,
                                    )
                            except Exception as e:
                                cprint(f"[be-plan] exception: {e}", fg='yellow', dim=True)
                                od = None
