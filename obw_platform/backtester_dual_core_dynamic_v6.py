...
import numpy as np
import pandas as pd
from slippage_directional_model_v2 import predict_directional_slippage_bp
...
def build_row_from_iso(iso: str, i: int, open_: np.ndarray, high: np.ndarray, low: np.ndarray, close: np.ndarray, volume: np.ndarray, extras: Dict[str, np.ndarray]) -> Dict[str, Any]:
    row: Dict[str, Any] = {
        'datetime_utc': iso,
        'open': float(open_[i]),
        'high': float(high[i]),
...
def simulate(cfg: dict, ts_s: np.ndarray, close: np.ndarray, open_: Optional[np.ndarray] = None, high: Optional[np.ndarray] = None, low: Optional[np.ndarray] = None, volume: Optional[np.ndarray] = None, extras: Optional[Dict[str, np.ndarray]] = None, market_symbol: str = 'ENA/USDT:USDT', model_override: Optional[dict] = None, export_curves: bool = False):
    ...
    iso_arr = [__import__('datetime').datetime.utcfromtimestamp(int(ts)).strftime('%Y-%m-%dT%H:%M:%S+00:00') for ts in ts_s]
    for i, ts in enumerate(ts_s):
        row = build_row_from_iso(iso_arr[i], i, open_, high, low, close, volume, extras)
        px = float(row['close'])
