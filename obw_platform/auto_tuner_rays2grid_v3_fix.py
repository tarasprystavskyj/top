...
def do_grid(base_cfg, limit_bars, params, prefix, session_dir, log_csv, weights, min_trades, target_trades, jobs):    # Expand search lists with current+seed inclusion
    ...
    def _process_result(vec, res):
        ...
        res["score"] = score
        eq_val = res.get("equity_end")
        try:
            eq_val_num = float(eq_val) if eq_val is not None else None
        except Exception:
            eq_val_num = None
        if eq_val_num is not None and (
            local_best_equity is None or eq_val_num > local_best_equity
        ):
            local_best_equity = eq_val_num
    ...
    def _emit_progress_if_ready():
        ...
        while next_emit_row < len(row_values) and row_done[next_emit_row]:
            ...
            is_last = next_emit_row == len(row_values) - 1
            text = f"{pct:.1f}%, " if not is_last else f"{pct:.1f}%"
            print(text, end="" if not is_last else "\n", flush=True)
            next_emit_row += 1
