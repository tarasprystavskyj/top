...
def _run_live(cfg: Dict[str, Any], args):
    # Prefer the dual live runner when cfg declares long+short strategy classes.
    if _is_dual_cfg(cfg):
        try:
    ...
    # Fallback / classic single-leg runner.
    try:
...
def main():
    ...
    ap.add_argument("--results-dir", default="", help="Results directory (default: auto under _reports/_live)")
    ap.add_argument("--limit-bars", type=int, default=0)

    # Session & cache
    ap.add_argument("--session-db", default="", help="Session DB path (default: session.sqlite in results dir)")
    ap.add_argument("--cache-out", default="", help="Cache output path (default: combined_cache_session.db in results dir)")
    ...
    ap.add_argument("--prewarm-bars", type=int, default=None, help="Number of historical bars to fetch for indicator warm-up")
    ap.add_argument("--prewarm-hours", type=int, default=None, help="Minimum history in hours for warm-up (converted to bars if provided)")
    ...
    if args.orders_db:
        os.environ["RS_ORDERS_DB"] = args.orders_db
    if args.session_db:
        os.environ["RS_SESSION_DB"] = args.session_db
    if args.cache_out:
        os.environ["RS_CACHE_OUT"] = args.cache_out
    if args.hour_cache:
        os.environ["RS_HOUR_CACHE"] = args.hour_cache
