...
def main():
    ap = argparse.ArgumentParser(description="Ultra-fast backtester (CSV + optional plots + DD/monotonicity) with adaptive sizing hooks")
    ...
    base_notional   = float(portfolio.get("position_notional", 20.0))
    ...
    # sizer clamp (if strategy exposes size_mult, ми обмежимо його межами із cfg.sizer)
    sizer_cfg = (cfg.get("sizer") or {}) if isinstance(cfg, dict) else {}
    min_mult = float(sizer_cfg.get("min_mult", 0.5))
    max_mult = float(sizer_cfg.get("max_mult", 1.8))
    ...
    for t, bucket in slices:
        ...
        if positions:
            for sym, pos in list(positions.items()):
                ...
                if hit:
                    ...
                    # equity update uses per-position notional
                    equity *= (1.0 + (pnl * pos.notional / equity))
                    ...
                    tr_rows.append({
                        ...
                        "realized_pnl": net_ret * pos.notional,
                    })
                    # feed return to strategy's sizer if available
                    try:
                        if hasattr(strat, "sizer") and hasattr(strat.sizer, "on_trade_close"):
                            strat.sizer.on_trade_close(net_ret)
                    except Exception:
                        pass
        ...
        # context with current dd & cfg (для сайзера у стратегії)
        current_dd = max(0.0, (equity_peak - equity) / max(equity_peak, 1e-12))
        shared_ctx = SimpleNamespace(cfg=cfg, current_dd=current_dd, equity_now=equity, equity_peak=equity_peak)

        # current open exposure
        def exposure_sum():
            return sum((p.notional for p in positions.values()), 0.0)
        ...
        for _, idx in best:
            ...
            # size multiplier (if strategy supplies it); clamp by cfg.sizer
            try:
                size_mult = float(getattr(sig, "size_mult", 1.0))
            except Exception:
                size_mult = 1.0
            if not (size_mult == size_mult):  # NaN check
                size_mult = 1.0
            if max_mult > 0 and min_mult > 0:
                if size_mult < min_mult: size_mult = min_mult
                if size_mult > max_mult: size_mult = max_mult
            new_notional = base_notional * size_mult

            # exposure cap
            if exposure_sum() + new_notional > max_notional_frac * equity:
                continue
            ...
            if sig.side=="LONG":
                ...
                sl = close + sl_mult*atr_abs; tp = close - tp_mult*atr_abs
            positions[sym] = Position(sig.side, close, sl, tp, new_notional, size_mult)
            pos_time[sym] = t

    # ===== Mark-to-market close of all =====
    if slices:
        last_t = slices[-1][0]
        ...
        for sym, pos in list(positions.items()):
            ...
            tr_rows.append({
                ...
                "size_mult": pos.size_mult, "notional": pos.notional,
                "realized_pnl": net_ret * pos.notional
            })
            try:
                if hasattr(strat, "sizer") and hasattr(strat.sizer, "on_trade_close"):
    ...
    if args.export_csv:
        if tr_rows:
            cols = ["symbol","side","entry_time","exit_time","entry","exit","tp","sl","reason","gross_return","net_return","size_mult","notional","realized_pnl"]
